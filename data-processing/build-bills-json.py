	...
	try:
		for action in data['actions']:
			if action['type'] == ['governor:vetoed']: #find if a governor:vetoed action exist
				bill_status = 'vetoed'
				if 'Vetoed' not in categories:
					categories.append('Vetoed')
				enddate = action['date']
			if action['type'] == ['governor:signed'] or 'Governor approval' in action['action']: #find if a governor:signed action exists
				bill_status = 'signed'
				enddate = action['date']
...
#	else:
	...
	if senate_data != []: #only update the votes if we actually have data
        ...
        #change status of pending bills
...
f = open('bills.json', 'a')
f.write(add_to_json(signedbills+vetoedbills+pendingbills) + "\n")
f.close()